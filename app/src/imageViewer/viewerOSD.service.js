var config_json = require("../../config/config.json"); //EDIT
...
(function () {
   ...
   angular
      ...
      .service("imageViewerHandler", [
         ...
         "imageScrollMap",
         "parsedData",
         function (evtInterface, imageScrollMap, parsedData) {
            ...
            viewerHandler.openPage = function (page) {
               ...
               if (pageValue !== currPage) {
                  viewerHandler.scope.$apply(function () {
                     evtInterface.updateState(
                        "currentPage",
                        pageValue !== "" ? pageValue : currPage
                     );
                  });
               }
            };
               ...
               try {
                  if (event.immediately === undefined) {
                     ...
                     if (viewerHandler.viewer.viewport.getZoom() === 1) {
                        ...
                        var oldBounds =
                           viewerHandler.viewer.viewport.getBounds();
                        if (newY > oldY) {
                           newPage = imageScrollMap.mapDown(
            ...
            function convertZoneToOSD(zone) {
               ...
               return new OpenSeadragon.Rect(
                  tmp.x / ImageNormalizationCoefficient,
                  tmp.y / ImageNormalizationCoefficient,
                  tmp.width / ImageNormalizationCoefficient,
                  tmp.hight / ImageNormalizationCoefficient
               );
            }
            ...
            viewerHandler.moveToZone = function (zone) {
               ...
               var currentBounds =
                  viewerHandler.viewer.viewport.getBounds(true);
               var normalizedZoney = zone.uly / ImageNormalizationCoefficient;
               var newY =
                  zone.uly / ImageNormalizationCoefficient < oldCenter.y
                     ? oldCenter.y
                     : zone.uly / ImageNormalizationCoefficient;
            ...
            viewerHandler.showHotSpot = function (zones) {
               var ht = parsedData.getHotSpots(); //recupero dati hotspot
               console.log(ht);
               var rectObjs = [];
               for (var i = 0; i < zones.length; i++) {
                  var r = new OpenSeadragon.Rect(
                     ...
                     (zones[i].lrx - zones[i].ulx) /
                        ImageNormalizationCoefficient,
                     (zones[i].lry - zones[i].uly) /
                        ImageNormalizationCoefficient
                  );
               ...
               for (var j = 0; j < zones.length; j++) {
                  var idImg = zones[j].corresp.substring(1); //id immagine
                  var urlimg = ht[idImg].images[0].attributes[0].value; //recupero dati immagine da id e ht
                  ...
                  hrefElt.dataset.img = urlimg; //creo attributo in cui passo l'id immagine al visualizzatore di popup
                  /*var hsImg = document.createElement("img");
                  hsImg.src = "../../../data/images/hotspot/" + urlimg;
                  hsImg.className = "hotspot-image";
                  hsImg.id = "hsi-" + id;
                  hrefElt.append(hsImg);
*/
            ...
            var showDivHotSpot = function (elem) {
               if (!divHotSpotToggle) {
                  ...
                  var topLeft =
                     viewerHandler.viewer.viewport.pointFromPixel(point1);
                  var bottomRight =
                     viewerHandler.viewer.viewport.pointFromPixel(point2);
                  var DivTopLeft = 0;
                  if (topLeft.x <= 0.4) {
                     DivTopLeft =
                        topLeft.x + (bottomRight.x - topLeft.x) + 0.05;
                  } else {
                  ...
                  divElt.draggable = "true";

                  var divTitleElt = document.createElement("div");

                  divTitleElt.id =
                     "div-title-hotspot-overlay_selected-" + elem.dataset.id;
                  ...
                  divBodyElt.id =
                     "div-body-hotspot-overlay_selected-" + elem.dataset.id;
                  divBodyElt.className = "hotspot-dida-body";

                  divBodyElt.innerHTML = elem.dataset.content;
                  //creo elemento immagine hotspot
                  var hsImg = document.createElement("img");
                  var IIIFSourceHotspot = config_json.IIIFSourceHotspot; //variabile globale per gli hotspot
                  hsImg.src = IIIFSourceHotspot + elem.dataset.img + "/full/max/0/default.jpg"; 
                     //"../../../data/images/hotspot/" + elem.dataset.img; //costruisco path da attributo passato
                  hsImg.className = "hotspot-image";
                  hsImg.id = "hsi-" + elem.dataset.id;

                  divElt.appendChild(divTitleElt);
                  divElt.appendChild(divBodyElt);
                  divElt.appendChild(hsImg); //appendo blocco immagine al contenitore
            ...
            var hiddenDivHotSpot = function (elem) {
               try {
                  ...
                  viewerHandler.viewer.removeOverlay(
                     "div-hotspot-overlay_selected-" + id
                  );
               } catch (error) {
            ...
            viewerHandler.hiddenHotSpot = function (zones) {
               try {
                  for (var i = 0; i < zones.length; i++) {
                     viewerHandler.viewer.removeOverlay(
                        "hotspot-overlay_selected-" + zones[i].id
                     );
                  }
               } catch (error) {
                  console.error("no hotspot overlay", error);
            ...
            viewerHandler.lineZone = function (zone) {
               ...
               elt.onmouseover = function () {
                  ...
                  var currentViewMode =
                     evtInterface.getState("currentViewMode");

                  if (ITLactive && currentViewMode === "imgTxt") {
                     this.className += " selectedHighlight";
                     var elemsInLine = document.querySelectorAll(
                        "[data-line='" + this.dataset["lb"] + "']"
                     );
                     if (elemsInLine.length == 0) {
                        var dataLine = this.dataset["lb"].replace(
                           "reg",
                           "orig"
                        );
                        elemsInLine = document.querySelectorAll(
                           "[data-line='" + dataLine + "']"
                        );
                     }
               ...
               elt.onmouseout = function () {
                  ...
                  if (ITLactive && currentViewMode === "imgTxt") {
                     ...
                     for (var i = 0; i < elemsInLine.length; i++) {
                        elemsInLine[i].className =
                           elemsInLine[i].className.replace(" lineHover", "") ||
                           "";
                     }
                  }
               };
