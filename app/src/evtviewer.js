...
/*----------------------------link image text only diplomatic edition fix----------------------------------*/

/* N.B. Da riabilitare in caso l'edizione interpretativa venga reintrodotta */
/*var isInterpretative = false;
function linkInterpretative(){
   var d_edition = document.getElementsByClassName(`diplomatic_edition_sel`);
   var i_edition = document.getElementsByClassName(
      `interpretative_edition_sel`
   );
   var linkerTxt = document.querySelector(
      '[title="BUTTONS.IMAGE_TEXT_LINKING"]'
   );
   if (isInterpretative){ //disabilita tasto link se edizione interpretativa è selezionata
      linkerTxt.classList.add("disable");
   }
   i_edition[0].addEventListener("click", function (e) {
      e.preventDefault();
      linkerTxt.classList.add("disable");
      isInterpretative = true;
   });
   d_edition[0].addEventListener("click", function (e) {
      e.preventDefault();
      linkerTxt.classList.remove("disable");
      isInterpretative = false;
   });
}
...
setTimeout(function(){
   var btnImageText = document.querySelector(
      '[title="VIEW_MODES.IMAGE_TEXT"]'
   );
   btnImageText.addEventListener("click", linkInterpretative);
}, 1000);*/

//Assegna un link target al tasto Viscoll custom (vedi navBar.directive.tmpl.html)
setTimeout(function () {
   var viscollBtn = document.querySelector('[ng-if="vm.config.viscollButton"]');
   viscollBtn.addEventListener('click', function() {
      var win = window.open("https://vceditor.library.upenn.edu/project/63d3d5a91650410001620f5e/viewOnly", '_blank');
      win.focus();
   }, false);
}, 3000);

//sistemazione tasto hotspot attivo anche quando hotspot inattivi

/*var areHotspotsActive = false;
function resetHotspotButton(){
   var hotspotButton = document.querySelector('[title="Hotspots"]');
   var pagesDropdown = document.querySelector('[data-id="page_mainText"]');

   hotspotButton.addEventListener("click", function(e){
      e.preventDefault();
      console.log("HOTSPOT BUTTON PRESSED");
      areHotspotsActive = true;
   });

   pagesDropdown.addEventListener("click", function(e){
      e.preventDefault();
      console.log("DROPDOWN PAGES PRESSED");
      hotspotButton.classList.remove("buttonSwitch-active");
   })
