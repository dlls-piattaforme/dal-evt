...
angular
   ...
   .service("evtParser", [
      ...
      "parsedData",
      "config",
      function ($q, xmlParser, parsedData, config) {
         var parser = {};
         var idx = 0;
         ...
         var defPageElement = "pb",
            ...
            possibleNamedEntitiesListsDef =
               "<listPlace>, <listPerson>, <listOrg>, <list>",
            defImageList = "image";
         var viscollDefs = {
            leaf: "leaf",
            leafMode: "mode",
         ...
         //VARIABILI AUSILIARE 2 COLONNA
         let cb = 0; //COLUMN BREAK FLAG (a 0 per cb a inizio codifica)
         let pb = 0; //PAGE BREAK FLAG
         let saltalinea = 0; //IMPEDISCE AGGANCIO CLASSE SECONDCOLUMN SUL TAG L IN CUI è PRESENTE IL CB
         let currentLine; //MEMORIZZA L CORRENTE
         let lastColumnline; //MEMORIZZA ULTIMA L IN CUI C'E UN CB
               ...
               if (
                  ...
                  if (
                     ...
                     newElement.className =
                        element.tagName !== undefined ? element.tagName : "";
                     ...
                     //INIZIO MODIFICHE 2 COLONNA

                     if (tagName === "lb") {
                        newElement.id = element.getAttribute("xml:id");
                        newElement.cl = element.getAttribute("style");
                        if (newElement.cl == "lbr") {
                           //diventa lbr
                           //aggiungo classe custom per spostare i numeri di riga dove marcati
                           newElement.className = "lb wordCb";
                        }
                     ...
                     if (tagName === "l") {
                        //recupero attributo style (in caso pbr)
                        newElement.pb = element.getAttribute("style");
                        currentLine = element.getAttribute("n");
                        //controllo se è gia scattato un column break (parte da 0 perchè a inizio documento c'è un cb che fa andare il contatore a 1)
                        if (cb > 1) {
                           //salto la prima iterazione, non voglio spostare tutto il line in cui è presente il cb
                           if (saltalinea > 0) {
                              newElement.id = element.getAttribute("xml:id");
                              //tutti i line successivi vengono spostati con questa classe
                              newElement.className = "l secondColumn";
                           }
                           //ho saltato la prima iterazione (sono dentro un if) quindi incremento saltalinea
                           saltalinea = cb;
                           //a fine pagina resetto le variabili
                           if (newElement.pb == "pbr") {
                              cb = 1;
                              saltalinea = 0;
                           }
                        } else {
                     ...
                     if (tagName === "choice") {
                        if (cb > 1) {
                           newElement.id = element.getAttribute("xml:id");
                           newElement.cb = element.getAttribute("style");
                           //se nel line è presente un cb, tutti i choice di quel line marcati con lo style wbr si spostano con una classe custom
                           if (newElement.cb == "wbr") {
                              //wordCb = 1;
                              newElement.className = "choice choice2col";
                           } else {
                              //test alternativa wbr
                              if (
                                 currentLine == lastColumnline &&
                                 saltalinea > 0
                              ) {
                                 newElement.className = "choice choice2col";
                              }
                     ...
                     if (tagName === "word") {
                        if (cb > 1) {
                           newElement.id = element.getAttribute("xml:id");
                           newElement.className = "word choice2col";
                        } else {
                           //console.log('firstCOl')
                        }
                     }
                     ...
                     //let cb=1;
                     if (tagName === "cb") {
                        lastColumnline = currentLine;
                        console.log("cb" + cb);
                        //incremento column break
                        cb = cb + 1;
                        newElement.id = element.getAttribute("xml:id");
                        newElement.className = "cb";
                        newElement.classList.add("cb" + cb);
                        newElement.appendChild(document.createElement("br"));
                     }

                     if (tagName === "pb") {
                        console.log("pb" + pb);
                        //incremento page break
                        pb = pb + 1;
                        newElement.id = element.getAttribute("xml:id");
                        newElement.className = "pb";
                        newElement.classList.add("pb" + pb);
                        newElement.appendChild(document.createElement("br"));
                        if (pb > 0) {
                           cb = 0;
                        }
     ...
     */
         ...
         parser.parseLine = function (doc, lineNode, options) {
            ...
            if (lineNum && lineNum !== "") {
               ...
               var parsedElement = parser.parseXMLElement(
                  doc,
                  newElement,
                  options
               );
            ...
            angular.forEach(
               ...
               function (element) {
                  ...
                  if (element.getAttribute("facs")) {
                     var IIIFSourcePage = config_json.IIIFSourcePage; //variabile globale per i facs
                     newPage.source = IIIFSourcePage + element.getAttribute("facs");
                  } else {
     ...
     */
          ...
          * @author CDP
          */
         parser.parseTextForEditionLevel = function (
            pageId,
            docId,
            editionLevel,
            docHTML
            ...
            balancedHTMLString = balancedHTMLString.replace(
               />\s+</g,
               ">__SPACE__<"
            );
            ...
            if (doc !== undefined) {
               ...
               if (invalidLbsSuffix) {
                  ...
                  while (k < lbs.length) {
                     ...
                     if (
                        lbNodeId &&
                        lbNodeId !== null &&
                        lbNodeId.indexOf(invalidLbsSuffix) >= 0
                     ) {
               ...
               while (k < Gs.length) {
                  ...
                  if (ref && ref !== "") {
                     ...
                     edition =
                        edition === "interpretative" ? "normalized" : edition;
                     if (
                        parser.isNestedInElem(gNode, "abbr") ||
                        parser.isNestedInElem(gNode, "orig")
                     ) {
                        edition = "diplomatic";
                     }
                     var glyphMappingForEdition =
                        parsedData.getGlyphMappingForEdition(ref, edition);
                     if (glyphMappingForEdition) {
                        glyphNode.appendChild(
                           angular.element(glyphMappingForEdition.element)[0]
                        );
                     }
                  }
                  if (glyphNode) {
                     //TODO Creare direttiva apposita per GLYPHs
                     gNode.parentNode.insertBefore(
                        glyphNode,
                        gNode.nextSibling
                     );
                  }
               ...
               angular.forEach(docDOM.children, function (elem) {
                  ...
                  elem.parentNode.replaceChild(
                     parser.parseXMLElement(doc, elem, {
                        skip: skip,
                     }),
                     elem
                  );
               ...
               editionText =
                  "<span> {{ 'TEXT_NOT_AVAILABLE' | translate }}</span>";
            }
